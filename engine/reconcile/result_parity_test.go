...
func TestExecutionPathParity_SingleSourceRows(t *testing.T) {
	...
	t.Run("streaming_disk", func(t *testing.T) {
		index, err := index.NewDiskIndex(t.TempDir())
		if err != nil {
			t.Fatal(err)
		}
...
func marshalParityResult(t *testing.T, result *Result) []byte {
	...
	writer := output.NewJSONWriter(&buffer)
...
func runParityWriter(t *testing.T, leftSource, rightSource string, run func(*output.JSONWriter) error) []byte {
	t.Helper()
	var buffer bytes.Buffer
	writer := output.NewJSONWriter(&buffer)
