...
- `ReconcilePartitionedMultiSourceWithOptions` (bounded-memory) stages the left CSV once, partitions each counterpart with the same resolved selector, and carries unmatched left rows through on-disk manifests between ordered passes. `ReconcilePartitionedMultiSourceWithOptionsAndTelemetry` adds the same telemetry options as the single-counterpart partitioned API.

The CLI (`internal/cli/reconcile.go`) calls the existing single-counterpart path unchanged when `len(pair.Counterparts()) == 1`, and switches to the multi-source path for `rights` pairs with more than one counterpart. Explicit `backend: partitioned` uses the disk-backed coordinator; resource-budgeted `auto` can select it when every counterpart is eligible, while no-budget `auto` retains its existing memory/disk behavior. `--audit` is not yet supported in the multi-source path, but `--progress` and `--progress-out` cover every counterpart pass. `--right-file` does not apply because each counterpart resolves its file via its source's `file_pattern`.
...
**Batch operation.** The `one_to_many` pass requires all rows for a group to be in memory simultaneously. With the `partitioned` backend, the CLI hashes the configured group key and runs the batch operation one partition at a time; otherwise it uses the whole-file in-memory batch path. `--audit` is not yet supported with grouped passes. Writers that do not support grouped events (`csv`, `table`) receive a warning on stderr; use `--format=json` or `--format=ndjson` to capture the full output including `grouped_matched`, `grouped_amount_diff`, `grouped_timing_diff`, and `ambiguous_groups`.
