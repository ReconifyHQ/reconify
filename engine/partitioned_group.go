...
func partitionCSVWithSidecars(ctx context.Context, input, keyCol, prefix string, n int, progress func(int)) (groupedPartitionFiles, error) {
	if n < 2 {
		return groupedPartitionFiles{}, fmt.Errorf("partition count must be at least 2 (got %d)", n)
	}
	in, err := os.Open(input) // #nosec G304 -- input is an explicit caller-selected reconciliation file.
	if err != nil {
		return groupedPartitionFiles{}, err
	}
	...
	if keyIdx < 0 {
		return groupedPartitionFiles{}, fmt.Errorf("partition key column %q not found", keyCol)
	}
	...
	for rowNum := 2; ; rowNum++ {
		...
		if err != nil {
			return groupedPartitionFiles{}, fmt.Errorf("row %d: %w", rowNum, err)
		}
		key := ""
	...
	return groupedPartitionFiles{data: dataPaths, rows: rowPaths, count: rows}, nil
}
