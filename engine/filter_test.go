...
func TestFilteredWriter_ReconcileIntegration_ExceptionsOnly(t *testing.T) {
	left := []Transaction{
		makeTx("l1", 100, "MATCH"),
		makeTx("l2", 200, "DIFF"),
		makeTx("l3", 300, "ONLY-L"),
	}
	right := []Transaction{
		makeTx("r1", 100, "MATCH"),
		makeTx("r2", 250, "DIFF"),
		makeTx("r3", 400, "ONLY-R"),
	}
	inner := &captureWriter{}
	w := WrapWithResultMode(inner, config.ResultModeExceptionsOnly, "run-42")
	result, err := Reconcile("pair", "left", "right", left, right, config.Pair{DateWindow: "0d"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestFilteredWriter_ReconcileIntegration_SummaryOnly(t *testing.T) {
	left := []Transaction{makeTx("l1", 100, "MATCH"), makeTx("l2", 200, "ONLY-L")}
	right := []Transaction{makeTx("r1", 100, "MATCH")}
	inner := &captureWriter{}
	w := WrapWithResultMode(inner, config.ResultModeSummaryOnly, "")
	result, err := Reconcile("pair", "left", "right", left, right, config.Pair{DateWindow: "0d"})
	if err != nil {
...
func TestSummaryCurrencyField(t *testing.T) {
	left := []Transaction{makeTx("l1", 100, "MATCH")}
	right := []Transaction{makeTx("r1", 100, "MATCH")}
	result, err := Reconcile("pair", "left", "right", left, right, config.Pair{DateWindow: "0d"})
	if err != nil {
		t.Fatal(err)
...
func TestSummaryRunIDFromOptions(t *testing.T) {
	left := []Transaction{makeTx("l1", 100, "REF")}
	right := []Transaction{makeTx("r1", 100, "REF")}
	result, err := Reconcile("pair", "left", "right", left, right, config.Pair{DateWindow: "0d"},
		ReconcileOptions{RunID: "test-run-id"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSummaryResultModeFromOptions(t *testing.T) {
	...
	right := []Transaction{makeTx("r1", 100, "REF")}
	result, err := Reconcile("pair", "left", "right", left, right, config.Pair{DateWindow: "0d"},
		ReconcileOptions{ResultMode: config.ResultModeExceptionsOnly})
	if err != nil {
		t.Fatal(err)
	}
